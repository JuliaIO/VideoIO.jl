...
function av_stream_set_recommended_encoder_configuration(s,configuration)
    ccall((:av_stream_set_recommended_encoder_configuration,libavformat),Cvoid,(Ptr{AVStream},Ptr{UInt8}),s,configuration)
end
...
function av_probe_input_buffer2(pb,fmt,url,logctx,offset::Integer,max_probe_size::Integer)
    ccall((:av_probe_input_buffer2,libavformat),Cint,(Ptr{AVIOContext},Ptr{Ptr{AVInputFormat}},Ptr{UInt8},Ptr{Cvoid},UInt32,UInt32),pb,fmt,url,logctx,offset,max_probe_size)
end

function av_probe_input_buffer(pb,fmt,url,logctx,offset::Integer,max_probe_size::Integer)
    ccall((:av_probe_input_buffer,libavformat),Cint,(Ptr{AVIOContext},Ptr{Ptr{AVInputFormat}},Ptr{UInt8},Ptr{Cvoid},UInt32,UInt32),pb,fmt,url,logctx,offset,max_probe_size)
end

function avformat_open_input(ps,url,fmt,options)
    ccall((:avformat_open_input,libavformat),Cint,(Ptr{Ptr{AVFormatContext}},Ptr{UInt8},Ptr{AVInputFormat},Ref{Ptr{AVDictionary}}),ps,url,fmt,options)
end
