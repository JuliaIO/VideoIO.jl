...
function _get_fc(file::String) # convenience function for `get_duration` and `get_start_time`
    v = open(file)
    return v.apFormatContext[1]
end
get_duration(fc::Ptr{AVFormatContext}) = Dates.Millisecond(fc[].duration) # this is a bit risky: if AV_TIME_BASE ≠ 1e6 then this conversion will give false results, or if `fc.duration` is not a whole number then this will result in an InexactError. I'll add the appropriate checks if you'll tell me that either event or both are possible.
...
get_start_time(fc::Ptr{AVFormatContext}) = Dates.unix2datetime(fc[].start_time_realtime)
...
get_time_duration(fc::Ptr{AVFormatContext}) = (get_start_time(fc), get_duration(fc))
