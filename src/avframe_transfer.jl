...
const VIO_DEF_ELTYPE_PIX_FMT_LU = Dict{DataType, Int32}(
    (strip_interpretation(v) => k for (k, v) in pairs(VIO_PIX_FMT_DEF_ELTYPE_LU))
)
...
VioColorspaceDetails() = VioColorspaceDetails(AVCOL_PRI_UNSPECIFIED,
                                              AVCOL_TRC_UNSPECIFIED,
                                              AVCOL_SPC_UNSPECIFIED,
                                              AVCOL_RANGE_UNSPECIFIED)

const VIO_DEFAULT_COLORSPACE_DETAILS = VioColorspaceDetails(AVCOL_PRI_BT709,
                                                            AVCOL_TRC_IEC61966_2_1,
                                                            AVCOL_SPC_RGB,
                                                            AVCOL_RANGE_JPEG)

const VIO_DEFAULT_TRANSFER_COLORSPACE_DETAILS =
    Dict{Cint,  VioColorspaceDetails}(
...
function _vio_determine_best_pix_fmt(src_pix_fmt, avarray_dst_pix_fmts =
    ...
    dst_pix_fmt, loss_ptr[]
end

is_pixel_type_supported(pxfmt) = haskey(VIO_PIX_FMT_DEF_ELTYPE_LU, pxfmt)

is_eltype_transfer_supported(::Type{T}) where T =
    haskey(VIO_DEF_ELTYPE_PIX_FMT_LU, strip_interpretation(T))

get_transfer_pix_fmt(::Type{T}) where T =
    VIO_DEF_ELTYPE_PIX_FMT_LU[strip_interpretation(T)]
...
@inline function unsafe_store_uint16_in_le!(data, x, offset)
    ...
    unsafe_store!(data, msb, offset + 1)
end
...
function transfer_img_buf_to_frame!(frame, img::AbstractArray{UInt8},
                                    scanline_major)
    ...
    if pix_fmt == AV_PIX_FMT_GRAY8
        ...
        if scanline_major
            ...
            @inbounds for r = 1:height
                line_offset = (r-1) * ls
                @simd for c = 1:width
                    @preserve frame unsafe_store!(data_p, img[r,c],
                                                  line_offset + c)
                end
            end
        end
...
function transfer_img_buf_to_frame!(frame, img::AbstractArray{UInt16},
                                    scanline_major)
    bytes_per_sample = 2
    pix_fmt = frame.format
    ...
    """)
            ...
            @preserve frame transfer_img_bytes_to_frame_plane!(datap,
                                                               img_raw,
                                                               width,
                                                               height,
                                                               ls,
            ...
            @inbounds for r in 1:height
                ...
                @simd for c in 1:width
                    ...
                    @preserve frame unsafe_store_uint16_in_le!(datap, img[r, c],
                                                        px_offset)
                end
            end
        end
...
function transfer_img_buf_to_frame!(frame, img::AbstractMatrix{RGB{N0f8}},
                                    scanline_major)
    pix_fmt = frame.format
    if pix_fmt == AV_PIX_FMT_RGB24
        ...
        if scanline_major
            data_p = fdata[1]
            @preserve frame transfer_img_bytes_to_frame_plane!(data_p,
...
function transfer_sl_col_img_buf_to_frame!(frame, img::AbstractArray{RGB{N6f10}})
    ...
    @inbounds for h in 1:height
        ...
        @simd ivdep for w in 1:width
            ...
            line_poss = line_offsets .+ col_pos
            input_px = img[w, h]
            unsafe_store_uint16_in_le!(fdata[1], reinterpret(green(input_px)),
                                       line_poss[1])
            unsafe_store_uint16_in_le!(fdata[2], reinterpret(blue( input_px)),
                                       line_poss[2])
            unsafe_store_uint16_in_le!(fdata[3], reinterpret(red(  input_px)),
                                       line_poss[3])
        end
    end
end

function transfer_img_buf_to_frame!(frame, img::AbstractArray{RGB{N6f10}},
                                    scanline_major)
    if scanline_major
        transfer_img = img
    else
    ...
    transfer_sl_col_img_buf_to_frame!(frame, transfer_img)
end

# Fallback
transfer_img_buf_to_frame!(frame, img, scanline_major) =
    _unsuported_append_encode_type()
